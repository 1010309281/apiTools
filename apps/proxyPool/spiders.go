...
/*
 代理ip网站:
	1. 高可用全球免费代理IP库 https://www.freeip.top (已实现)
	2. 西刺免费代理IP https://www.xicidaili.com	(已实现)
	...
	12. 极速数据		http://www.superfastip.com
*/
...
var (
	spiderCron *cron.Cron // 爬虫定时器对象
)

// 运行爬虫
func RunSpiders() {
	spiderCron = utils.NewWithCron()
	registerSpiders(withFreeipSpider(), withXiciSpider(), withSuperipSpiderSpider())
}
...
func registerSpiders(spiders ...spiderOption) {
	for _, spider := range spiders {
		s := spider()
		if config.GetBool("proxyPoolApp::isStartRunSpider") {
			go startSingleSpider(s)
		}
		_, err := spiderCron.AddFunc(s.spiderTaskSpec(), func() {
			startSingleSpider(s)
		})
		if err != nil {
			logger.Echo.Errorf("create %s spider task fail, err: %v", s.spiderName(), err)
		}
		spiderCron.Start()
		logger.Echo.Infof("create %s spider task success", s.spiderName())
	}
}
...
func (s *freeipSpider) getProxyData() (err error) {
	...
	for i := 1; i <= s.pages; i++ {
		...
		if err != nil {
			logger.Echo.Errorf("run %s spider, page is: %d, err: %v", s.name, i, err)
			failCount++
			i--
	...
	logger.Echo.Infof("run %s spider end", s.name)
	return
}
...
func withFreeipSpider() spiderOption {
	return func() (spider spiderInterface) {
		freeip := &freeipSpider{
			...
			url:       "https://www.freeip.top/api/proxy_ips?page=",
			pages:     20,
			failCount: 10,
		}
		spider = freeip
...
func (s *xiciSpider) getProxyData(url string) {
	...
	for i := 1; i <= s.pages; i++ {
		...
		if err != nil {
			failCount++
			i--
			logger.Echo.Errorf("run %s spider,url: %s, err: %v", s.name, dataUrl, err)
...
func (s *superfipSpider) getProxyData() error {
	failCount := 0
	for i := 1; i <= s.pages; i++ {
		if failCount > s.failCount {
			logger.Echo.Errorf("%s site may have been hung up", s.name)
...
// 声明配置爬虫
func withSuperipSpiderSpider() spiderOption {
	return func() (spider spiderInterface) {
